...
void main() {
  ...
  test(
    'batch canonicalization is rejected so rows require explicit decisions',
    () async {
  ...
  test(
    'batch path cannot create a duplicate before row-level confirmation',
    () async {
      transactions.values['existing'] = _transaction('existing', amount: '12');
      final rows = [_row('duplicate', amount: '12')];
      final assessment =
      ...
      final result = await service.importBatch(_statement(), rows, 'source');
      expect(result, isA<ApplicationFailure<StatementImportSummary>>());
      expect(transactions.values.keys, contains('existing'));
      expect(
        transactions.values.keys,
        isNot(contains('statement-statement-row-duplicate')),
      );
  ...
  test(
    ...
    () async {
      ...
      final row = _row('one');
      final first = await service.save(row, 'source', allowCreateNew: true);
      final savedRow = statements.rows.single;
      final second = await service.save(
        savedRow,
        'source',
        allowCreateNew: true,
      );
      expect(first, isA<ApplicationSuccess<TransactionDto>>());
      expect(second, isA<ApplicationSuccess<TransactionDto>>());
  ...
  test('statement intake preserves missing currency and direction', () async {
    final row = _row('unknowns', currency: null, direction: null);
