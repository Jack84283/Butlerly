...
final class ButlerlyDatabase {
  ...
  static Future<void> _migrate(Database database, int from, int to) async {
    try {
      ...
      if (from < 15 && to >= 15) {
        for (final statement in Schema.migration15) {
          await database.execute(statement);
        }
      }
      if (from < 16 && to >= 16) {
        for (final statement in Schema.migration16) {
          await database.execute(statement);
        }
      }
      if (from < 17 && to >= 17) {
        for (final statement in Schema.migration17) {
          await database.execute(statement);
        }
      }
      if (from < 18 && to >= 18) {
        for (final statement in Schema.migration18) {
          await database.execute(statement);
        }
      }
      if (from < 19 && to >= 19) {
        for (final statement in Schema.migration19) {
          await database.execute(statement);
        }
      }
      if (from < 20 && to >= 20) {
      ...
      if (from < 21 && to >= 21) {
        for (final statement in Schema.migration21) {
          await database.execute(statement);
        }
      }
