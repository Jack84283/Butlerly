...
void main() {
  ...
  test(
    'merge does not resurrect a review issue deleted after backup',
    () async {
      ...
      final backup = File(
        path.join(fixture.root.path, 'review.butlerlybackup'),
      );
  ...
  test(
    ...
    () async {
      ...
      final backup = File(
        path.join(fixture.root.path, 'tombstone.butlerlybackup'),
      );
      await fixture.manager.createBackup(backup);

      final recreatedAt = DateTime.now().toUtc().add(
  ...
  test(
    'prepared-phase recovery uses directory state and clears restore context',
    () async {
      final fixture = await _Fixture.create();
      addTearDown(fixture.dispose);
