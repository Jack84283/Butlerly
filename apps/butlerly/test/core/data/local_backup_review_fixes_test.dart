...
void main() {
  ...
  test('merge restore preserves historical master-data updatedAt', () async {
    ...
    final backup = File(
      path.join(fixture.root.path, 'timestamps.butlerlybackup'),
    );
  ...
  test(
    'unproven post-commit cleanup state fails closed without deleting evidence',
    () async {
      final fixture = await _Fixture.create();
      addTearDown(fixture.dispose);
  ...
  test('backup package keeps pre-write database state', () async {
    ...
    final backup = File(
      path.join(fixture.root.path, 'snapshot.butlerlybackup'),
    );
  ...
  test(
    'merge purges newer unresolved duplicate state transactionally',
    () async {
      final fixture = await _Fixture.create();
      addTearDown(fixture.dispose);
      ...
      final backup = File(
        path.join(fixture.root.path, 'generated.butlerlybackup'),
      );
      await fixture.manager.createBackup(backup);
  ...
  test(
    'corrupt evidence fails before generated local state is purged',
    () async {
      final fixture = await _Fixture.create();
      addTearDown(fixture.dispose);
      final old = DateTime.utc(2025, 1, 1);
  ...
  test(
    'database failure rolls generated-state purge back with merge',
    () async {
      final fixture = await _Fixture.create();
      addTearDown(fixture.dispose);
      final old = DateTime.utc(2025, 1, 1);
      ...
      final backup = File(
        path.join(fixture.root.path, 'rollback.butlerlybackup'),
      );
      await fixture.manager.createBackup(backup);
  ...
  test(
    'concurrent edit during evidence staging keeps timestamp and wins merge',
    () async {
      final fixture = await _Fixture.create();
      addTearDown(fixture.dispose);
      final old = DateTime.utc(2025, 1, 1).toIso8601String();
      ...
      await fixture.database.database.update(
        'merchants',
        {
          ...
          'normalized_name': 'concurrent local edit',
        },
        where: 'id = ?',
  ...
  test(
    'merge chooses a safe suffix when evidence remap target is occupied',
    () async {
      final fixture = await _Fixture.create();
      addTearDown(fixture.dispose);
      final old = DateTime.utc(2025, 1, 1).toIso8601String();
      ...
      await occupiedRemap.writeAsString(
