...
class _MasterDataPageState extends State<MasterDataPage> {
  ...
  Future<_MasterData> _load() async {
    ...
    return _MasterData(
      ...
      tags.value,
      merchants.value,
      {'en': labelsAt(3), 'zh-Hans': labelsAt(4), 'es': labelsAt(5)},
      {'en': labelsAt(6), 'zh-Hans': labelsAt(7), 'es': labelsAt(8)},
    );
  }
...
final class _MasterData {
  ...
  String categoryLabel(Category value, String languageCode) {
    ...
    return categoryLabels[locale]?[value.id.value] ??
        categoryLabels['en']?[value.id.value] ??
        value.name;
  }

  String tagLabel(Tag value, String languageCode) {
    if (!value.id.value.startsWith('tag.')) return value.name;
    final locale = languageCode == 'zh' ? 'zh-Hans' : languageCode;
    return tagLabels[locale]?[value.id.value] ??
